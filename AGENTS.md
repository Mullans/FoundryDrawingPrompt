...
3. Run `node --test tests/`. For Foundry-facing or UI changes, also run `node tools/e2e-smoke.mjs` (Foundry already up on `:30000`, world `test-world`) and check GM/player browser consoles for errors. UI automation must type and click through the real UI, not call only the module API. The smoke run includes **layout geometry asserts** (`tools/e2e-layout-geometry.mjs`) so compose/review panel overflow and column overlap fail the e2e — unit tests alone cannot catch that.
